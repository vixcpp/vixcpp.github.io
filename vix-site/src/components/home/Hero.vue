        ...
        <div v-if="props.support" class="hero-support">
          ...
          <div v-if="props.support.meta?.length" class="hero-support-meta">
            ...
            <template v-for="(m, i) in props.support.meta" :key="i">
              <span>{{ m }}</span>
              <span v-if="i < props.support.meta.length - 1" class="sep"
                >•</span
              >
            </template>
          </div>
...
const active = computed(
  () =>
    tabs.value.find((t) => t.key === activeKey.value) || tabs.value[0] || null,
);
...
.meta-dot {
  ...
  height: 8px;
  border-radius: 999px;
  background: rgba(34, 197, 154, 0.85);
  box-shadow: 0 0 0 3px rgba(34, 197, 154, 0.18);
}
...
.btn.cta-main {
  background: linear-gradient(
    180deg,
    rgba(34, 197, 154, 0.22),
    rgba(34, 197, 154, 0.12)
  );
  border-color: rgba(34, 197, 154, 0.55);
  color: #ecfeff;
}
